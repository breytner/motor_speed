...
bool FastBus::get_direct_mem_ptr(tlm::tlm_generic_payload &trans, tlm::tlm_dmi &dmi_data){
	ensitlm::addr_t a = trans.get_address();
	addr_map_t::iterator it = addr_map.find(addr_range(a, a));
	if (it == addr_map.end()) {
		std::cerr << name() << ": no target at address "
	...
	trans.set_address(a - (*it).first.begin);
	return initiator[(*it).second]->get_direct_mem_ptr(trans, dmi_data);
}
...
void FastBus::end_of_elaboration() {
	...
	for (int i = 0; i < initiator.size(); ++i) {
		...
		if (!target) {
			std::cerr << name()
			          << initiator[i] << ": target is not a tlm_target_socket\n"
					  << std::endl;
			abort();
		}
