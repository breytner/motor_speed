...
tlm::tlm_response_status Memory::read(ensitlm::addr_t a, ensitlm::data_t &d) {
	...
	if (a >= m_size) {
		...
		// if ( std::string(name()).compare("counter_mem") )
		// 	std::cout << "READING " << std::dec << d << " IN COUNTER" << std::endl;


#ifdef DEBUG
		std::cout << name() << ": Read  access at " << std::showbase
		          << hex << a << " (Data: 0x" << d << ")" << std::endl;
#endif
...
tlm::tlm_response_status Memory::write(ensitlm::addr_t a, ensitlm::data_t d) {
	...
	if (a >= m_size) {
		...
		// if ( std::string(name()).compare("counter_mem") )
		// 	std::cout << "WRITING " << std::dec << d << " IN COUNTER" << std::endl;
