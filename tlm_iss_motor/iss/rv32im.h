 ...
 *
 * For now, interprets only the RV32IM instructions, which is good
 * enough for the goal I'm pursuing.
\*/

#ifndef _SOCLIB_RV32IM_ISS_H_
#define _SOCLIB_RV32IM_ISS_H_
...
/*\
 *  Rv32im Processor structure definition
\*/
namespace soclib {
namespace common {
	class Rv32imIss
	...
	private:
		...
		float               r_fpr[32]; // Floating Point Registers, just placeholders for now

		// Control and status registers, quite a load I must say
		// Not all of them are meaningfull at first, so I implemented the ones
		// that are used in the simple riscv-probe bare metal example
		uint32_t            r_csr[4096];
		...
		FILE               *dump;          // File to log instructions
	public:
		/*\
		 * The Rv32im has interrupt wires, I guess, but how many?
		\*/
