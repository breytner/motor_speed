...
SC_MODULE(FastBus), tlm::tlm_fw_transport_if<tlm::tlm_base_protocol_types>,
	...
	unsigned int transport_dbg(tlm::tlm_generic_payload &) {
		std::cerr << name() << "transport_dbg not implemented" << std::endl;
		abort();
	}

	tlm::tlm_sync_enum nb_transport_fw(
	    tlm::tlm_generic_payload &, tlm::tlm_phase &, sc_core::sc_time &) {
		std::cerr << name() << "nb_transport_fw not implemented" << std::endl;
		abort();
	}

	void invalidate_direct_mem_ptr(sc_dt::uint64, sc_dt::uint64) {
		std::cerr << name() << "invalidate_direct_mem_ptr not implemented"
		          << std::endl;
		abort();
	}
	...
	tlm::tlm_sync_enum nb_transport_bw(
	    tlm::tlm_generic_payload &, tlm::tlm_phase &, sc_core::sc_time &) {
		std::cerr << name() << "nb_transport_bw not implemented" << std::endl;
		abort();
	}
